...
func VerifyProxy() {
  ...
  HttpsCount = fetch_https_count()
  lock.Unlock()
  log.Printf("\r%s 代理验证结束, 当前可用IP数: %d(%d)\n", time.Now().Format("2006-01-02 15:04:05"), len(ProxyPool), HttpsCount)
  verifyIS = false
}

func fetch_https_count() int {
  count := 0
  for _, data := range ProxyPool {
    if data.Type == "HTTPS" {
      count += 1
    }
  }
